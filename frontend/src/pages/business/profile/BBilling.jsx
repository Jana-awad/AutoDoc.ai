...
import {
  changeBusinessPlan,
  clearBusinessBillingHistory,
  fetchBusinessBilling,
  fetchBusinessBillingHistory,
...
const BBilling = () => {
    ...
    async (signal) => {
      ...
      try {
        const [billingData, invoiceData, historyData] = await Promise.all([
          fetchBusinessBilling({ token, signal }),
          fetchBusinessInvoices({ token, signal }),
          fetchBusinessBillingHistory({ token, signal }),
        ]);
  ...
  const handleClearHistory = async () => {
    ...
    setError(null);
    try {
      await clearBusinessBillingHistory({ token });
      fetchData();
    } catch (err) {
  ...
  const handleChangePlan = async (planId) => {
    ...
    setError(null);
    try {
      await changeBusinessPlan({ token, planId });
      fetchData();
    } catch (err) {
  ...
  const plans = useMemo(() => {
    ...
    return billing.availablePlans.map((plan) => ({
      ...
      isCurrent:
        plan.isCurrent ||
        (billing?.planId && (plan.id === billing.planId || plan.planId === billing.planId)),
    }));
  }, [billing]);
                ...
                {invoices.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id || inv.number}>
                      ...
                      <td>
                        <span className={`b-badge ${status.toLowerCase() === "paid" ? "b-badge-active" : "b-badge-inactive"}`}>
                          {status}
                        </span>
                      </td>
                      <td>
                        {downloadUrl ? (
                          <a className="b-action-btn b-action-btn-edit" href={downloadUrl} target="_blank" rel="noreferrer">
                            Download
                          </a>
                        ) : (
                          <button className="b-action-btn b-action-btn-edit" disabled>
                            Download
                          </button>
                        )}
                      </td>
                ...
                {history.map((entry) => {
                  ...
                  return (
                    <tr key={entry.id || `${entry.type}-${entry.date}`}>
                      ...
                      <td>
                        <span className={`b-badge ${status.toLowerCase() === "paid" || status.toLowerCase() === "active" ? "b-badge-active" : "b-badge-inactive"}`}>
                          {status}
                        </span>
                      </td>
                    </tr>
