...
const BManageUsers = () => {
  ...
  const fetchUsers = useCallback(
    async (signal) => {
      ...
      setError(null);
      try {
        const data = await fetchBusinessUsers({ token, signal });
  ...
  const handleAddUser = async (e) => {
    ...
    try {
      ...
      await createBusinessUser({ token, data: payload });
  ...
  const handleRoleChange = async (userId, newRole) => {
    setError(null);
    try {
      await updateBusinessUser({ token, userId, data: { role: newRole } });
      fetchUsers();
    } catch (err) {
  ...
  const handleStatusToggle = async (user) => {
    ...
    setError(null);
    try {
      await updateBusinessUser({ token, userId: user.id, data: { status: nextStatus } });
      fetchUsers();
    } catch (err) {
  ...
  const handleRemove = async (userId) => {
    ...
    setError(null);
    try {
      await removeBusinessUser({ token, userId });
      fetchUsers();
    } catch (err) {
  ...
  const stats = useMemo(() => {
    ...
    const adminCount = users.filter((user) => String(user?.role || "").toLowerCase() === "business_admin").length;
