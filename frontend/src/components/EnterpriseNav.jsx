...
const LogoMark = () => (
  <svg viewBox="0 0 28 28" aria-hidden="true">
    <defs>
      <linearGradient id="logoGradientEnt" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0%" stopColor="#5ba3e0" />
        <stop offset="100%" stopColor="#1e3a5f" />
      </linearGradient>
    </defs>
    <rect x="2" y="2" width="24" height="24" rx="8" fill="url(#logoGradientEnt)" />
    <path
...
const enterpriseMegaMenus = {
  dashboard: {
    title: 'Dashboard',
    description: 'Enterprise workspace home and activity.',
    columns: [
      {
        heading: 'Pages',
        items: [
          { title: 'Dashboard', description: 'Overview and recent activity.', path: '/enterprise', icon: <DashboardIcon /> },
          { title: 'Documents hub', description: 'Upload and process documents.', path: '/enterprise/documents', icon: <MonitorIcon /> },
        ],
      },
    ],
  },
  templates: {
    title: 'Template & AI',
    description: 'Design, manage, and deploy extraction templates for your organization.',
    columns: [
      {
        heading: 'Pages',
        items: [
          { title: 'Templates overview', description: 'Overview of templates and AI extraction.', path: '/enterprise/templates-ai', icon: <TemplateIcon /> },
          { title: 'Template builder', description: 'Create and configure AI extraction templates.', path: '/enterprise/templates-ai/builder', icon: <PipelineIcon /> },
          { title: 'API & integrations', description: 'Keys, usage, and integrations.', path: '/enterprise/api', icon: <SparkIcon /> },
        ],
      },
    ],
  },
    ...
    columns: [
      {
        heading: 'Account',
        items: [
          { title: 'Profile', description: 'Your profile and preferences.', path: '/enterprise/profile', icon: <ClientsIcon /> },
          { title: 'Account settings', description: 'Organization details.', path: '/enterprise/profile/account', icon: <ShieldIcon /> },
          { title: 'Team users', description: 'Invite users and roles.', path: '/enterprise/profile/users', icon: <ShieldIcon /> },
        ],
      },
      {
        heading: 'Plans & billing',
        items: [
          { title: 'Billing', description: 'Invoices and payment.', path: '/enterprise/profile/billing', icon: <BillingIcon /> },
          { title: 'Subscription', description: 'Plan and limits.', path: '/enterprise/profile/billing', icon: <BillingIcon /> },
          { title: 'Workspace settings', description: 'Preferences and defaults.', path: '/enterprise/profile/settings', icon: <ShieldIcon /> },
        ],
      },
      {
        heading: 'Shortcuts',
        items: [
          { title: 'Dashboard', description: 'Back to home.', path: '/enterprise', icon: <DashboardIcon /> },
          { title: 'Documents', description: 'Process files.', path: '/enterprise/documents', icon: <PipelineIcon /> },
          { title: 'Templates', description: 'Manage extraction.', path: '/enterprise/templates-ai', icon: <TemplateIcon /> },
        ],
      },
    ],
  },
...
function buildCommandDestinations(navLinks) {
  const list = [];
  Object.values(enterpriseMegaMenus).forEach((m) => {
  ...
  list.push({
    ...
    title: 'Documents hub',
    hay: 'documents upload process files hub',
  });
  return list;
}
...
const EnterpriseNav = ({
  ...
  const navLinks = useMemo(
    () => [
      { name: 'Dashboard', path: '/enterprise', mega: 'dashboard' },
      { name: 'Template & AI', path: '/enterprise/templates-ai', mega: 'templates' },
      { name: 'Workspace', path: '/enterprise/profile', mega: 'workspace' },
      { name: 'Documents', path: '/enterprise/documents' },
      { name: 'API', path: '/enterprise/api' },
    ],
    []
  );
    ...
    >
      ...
      <div className="supernav-inner">
        <div className="supernav-left">
          <Link to="/enterprise" className="supernav-logo" aria-label="AutoDoc AI">
                  ...
                  {hasMega && (
                    <div className={`supernav-mega ${isOpen ? 'open' : ''}`} role="menu">
                      <div className="supernav-mega-header">
                        <h3>{enterpriseMegaMenus[link.mega].title}</h3>
                        <p>{enterpriseMegaMenus[link.mega].description}</p>
                      </div>
                      <div className="supernav-mega-grid">
                        {enterpriseMegaMenus[link.mega].columns.map((column, columnIndex) => (
              ...
              return (
                <div key={link.name} className="supernav-mobile-section">
                  ...
                  <div className={`supernav-mobile-submenu ${isOpen ? 'open' : ''}`}>
                    {enterpriseMegaMenus[link.mega].columns.flatMap((column) => column.items).map((item) => (
