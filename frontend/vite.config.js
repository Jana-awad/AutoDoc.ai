      ...
      '/api': {
        ...
        configure: (proxy) => {
          proxy.on('error', (err, req, res) => {
            console.warn('[Vite proxy] Backend request failed. Is the backend running on http://127.0.0.1:8000?', err.message);
          });
          proxy.on('proxyRes', (proxyRes, req, res) => {
            if (proxyRes.statusCode === 404 && req.url?.startsWith('/api/')) {
              console.warn('[Vite proxy] 404 for', req.url, '- Restart the backend so /v1/business/* routes are loaded.');
            }
          });
        },
      },
