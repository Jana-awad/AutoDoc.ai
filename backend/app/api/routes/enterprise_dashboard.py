"""Enterprise dashboard API: metrics, top users, activity, health, etc. (mirrors business dashboard)."""
...
router = APIRouter(prefix="/v1/enterprise/dashboard", tags=["enterprise-dashboard"])
...
def get_dashboard_metrics(
    ...
    remaining_quota = max(0, 50000 - api_calls_month) if api_calls_month else 50000
...
def get_top_users(
    ...
    """Top users in the client (enterprise)."""
    cid = _client_filter(db, current_user)
    if not cid:
    ...
    for i, u in enumerate(users):
        role_value = (u.role.value if hasattr(u.role, "value") else str(u.role)).strip().lower().replace(" ", "_")
        if role_value == "business_client_admin":
            role_value = "business_admin"
        out.append(
            DashboardTopUser(
                id=u.id,
                name=u.username,
...
def get_system_health(
    ...
    """System health (operational by default)."""
...
def get_ai_analytics(
    ...
    """AI processing analytics (derived from documents)."""
    cid = _client_filter(db, current_user)
...
def get_api_usage(
    ...
    return DashboardApiUsageResponse(
        requestsPerMinute=min(total // 30, 200),
        successRate=round(success_rate, 1),
...
def get_support_sla(
    ...
    """Support and SLA (Enterprise tier)."""
    return DashboardSupportSlaResponse(
        ...
        openTickets=0,
        resolvedTickets=0,
        contactEmail="enterprise@autodoc.example",
        ticketsUrl=None,
    )
